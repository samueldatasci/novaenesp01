...
def chart(kind="line", df=None, stacked=False, normalize=False, \
	...
	yvar='count', ylabel='', ylimit=(0, None), yAxisScale=None, yAxisScaleSymbol=None, \
	zvar=None, zlabel = "", \
	title=None, \
	grid=True, colormap='NOVAIMS', legend=True, xticks=None, yticks=None, figsize=(8,4.5), \
	dots = [],
	fontsize_ticks=8, fontsize_xlabel=12, fontsize_ylabel=12, fontsize_title=14, fontsize_legend=8):
	...
	if kind in ['bar', 'barh']:
		...
		ax = df_grouped.plot(kind=kind, stacked=stacked, \
				...
				colormap=colormap, figsize=figsize, rot=0, fontsize=12, legend=legend, width = 0.8,
		...
		ax = df_grouped.plot(kind=kind, stacked=stacked, \
				x=xvar, xlabel=xlabel, xlim=xlimit, \
				ylim=ylimit, ylabel=ylabel, \
				colormap=colormap, figsize=figsize, rot=0, fontsize=12, legend=legend,
				ax=None, subplots=False, xticks=xticks, yticks=yticks)
	...
	ax.xaxis.label.set_size(fontsize_xlabel)
	ax.yaxis.label.set_size(fontsize_ylabel)

	ax.set_title(title, fontsize=fontsize_title, pad=15)


	if legend:
		ax.legend(prop={'size': fontsize_legend})



	add_caption(fig, "samuel.santos@novaims.unl.pt", fontsize=6, ha="right")
...
def add_caption(fig, text, fontsize=6, ha="left"):
    """
    Adds explanatory text below the plot.
    """
